...
import { BookOpen, Layers3, Plus, Search } from 'lucide-react'
import { SchoolAdminShell } from '@/components/admin/SchoolAdminShell'
import { useLevelsQuery, useSubjectsQuery } from '@/hooks/useSchoolQueries'
...
export default function SubjectsPage() {
  ...
  const isError = levelsQuery.isError || subjectsQuery.isError
  const isFetching = levelsQuery.isFetching || subjectsQuery.isFetching
  const showInitialLoading =
    (levelsQuery.isPending || subjectsQuery.isPending) &&
    levels.length === 0 &&
    subjects.length === 0
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return subjects.filter((subject) => {
      const matchesLevel =
        levelFilter === 'all' || subject.level_uuid === levelFilter
      const matchesQuery =
        !query ||
        [subject.name, subject.code, subject.level?.name, subject.level?.code]
          .filter(Boolean)
        ...
        <div className="mx-auto max-w-[1500px]">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Create grade levels first, then add subjects like Math and English
                under each level.
              </p>
            </div>
          ...
          {!showInitialLoading && !hasLevels ? (
            <div className="mt-6 rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm text-blue-800">
              Create a level first (for example Grade 7), then you can add
              subjects under that level.
            </div>
          ) : null}
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              {levelFilter !== 'all' ? (
                ...
                <p className="text-xs text-slate-400">
                  Click a level to filter subjects
                </p>
              )}
            </div>
            {levels.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        >
                          {level.subjects_count ?? 0} subjects
                        </span>
                      </div>
                    </button>
                  )
          ...
          filteredSubjects.length === 0 ? (
            <div className="mt-6 rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center">
              ...
              <p className="mt-1 text-xs text-slate-400">
                Add subjects like Math or English under a grade level.
              </p>
              <Link
                to="/admin/subjects/create"
