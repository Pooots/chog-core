...
export default function CreateRoomSchedulePage() {
  const navigate = useNavigate()
  const { roomUuid } = useParams({
    from: '/admin/rooms/$roomUuid/schedules/create',
  })
  ...
  const takenSubjectUuids = useMemo(() => {
    ...
    for (const schedule of schedulesQuery.data ?? []) {
      ...
      if (schedule.subject_uuid) taken.add(schedule.subject_uuid)
    }
    return taken
  }, [schedulesQuery.data, dayOfWeek])

  const lastScheduleEndTime = useMemo(() => {
    const daySchedules = (schedulesQuery.data ?? [])
      .filter(
        (schedule) =>
          schedule.status === 'active' && schedule.day_of_week === dayOfWeek,
      )
      .sort((a, b) => a.end_time.localeCompare(b.end_time))

    return daySchedules.at(-1)?.end_time ?? null
  }, [schedulesQuery.data, dayOfWeek])
  ...
  const createSchedule = useCreateRoomScheduleMutation(roomUuid)
  ...
  useEffect(() => {
    if (!room || schedulesQuery.isPending) return

    const roomStart = room.start_time ?? '08:00'
    const roomEnd = room.end_time ?? '17:00'
    const nextStart = lastScheduleEndTime ?? roomStart

    const [h, m] = nextStart.split(':').map(Number)
    const next = new Date(2000, 0, 1, h, m + 60)
    const endCandidate = `${String(next.getHours()).padStart(2, '0')}:${String(next.getMinutes()).padStart(2, '0')}`
    const nextEnd =
      endCandidate > nextStart && endCandidate <= roomEnd
        ? endCandidate
        : roomEnd

    setStartTime(nextStart)
    setEndTime(nextEnd)
  }, [room, lastScheduleEndTime, schedulesQuery.isPending, dayOfWeek])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createSchedule.mutateAsync({
        ...
        subject_uuid: isClassPeriod ? subjectUuid || undefined : null,
        teacher_uuid: isClassPeriod ? teacherUuid || undefined : null,
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm text-slate-500">
              {room
                ? `Add a timed class period for ${room.name}.`
                : 'Add a timed class period for this room.'}
            </p>
            ...
            >
              ...
              <section className="space-y-4 border-t border-slate-100 pt-5">
                ...
                <p className="text-xs text-slate-400">
                  {lastScheduleEndTime
                    ? `Start time continues from the last ${selectedDayLabel} schedule ending at ${lastScheduleEndTime}.`
                    : `No schedule yet on ${selectedDayLabel}. Start time uses the room opening hour.`}
                </p>
                ...
                <label className="block text-sm font-medium text-slate-700">
                  ...
                  <p className="mt-1.5 text-xs text-slate-400">
                    Showing subjects for {room?.level?.name ?? 'this room grade'}{' '}
                    that are not yet scheduled on {selectedDayLabel}.
                  </p>
              ...
              <div className="flex items-center justify-between border-t border-slate-100 pt-5">
                ...
                <button
                  type="submit"
                  disabled={createSchedule.isPending || roomQuery.isError}
