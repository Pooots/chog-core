...
export default function EditFacultyPage() {
  ...
  const { facultyUuid } = useParams({
    from: '/admin/faculties/$facultyUuid/edit',
  })
  const facultyQuery = useFacultyQuery(facultyUuid)
  const typesQuery = useFacultyTypesQuery()
  const subjectsQuery = useSubjectsQuery()
  const updateFaculty = useUpdateFacultyMutation(facultyUuid)
  ...
  useEffect(() => {
    if (!faculty || !subjectsQuery.data || !initialized) return

    setSelectedSubjectUuids(
      subjectsQuery.data
        .filter((subject) => subject.faculty_uuid === faculty.uuid)
        .map((subject) => subject.uuid),
    )
  }, [faculty, subjectsQuery.data, initialized])
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateFaculty.mutateAsync({
        ...
        subject_uuids: selectedSubjectUuids,
      })
      ...
      if (axios.isAxiosError(requestError)) {
        ...
        setError(
          firstValidationError ??
            ...
            'Unable to update the faculty.',
        )
      } else {
        setError('Unable to update the faculty.')
      }
    }
  }
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <div className="mt-4">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Update faculty details and assigned subjects.
            </p>
          </div>

          {facultyQuery.isPending && !faculty ? (
            <p className="mt-6 text-sm text-slate-500">Loading faculty…</p>
          ) : null}

          {facultyQuery.isError ? (
            <div className="mt-6 rounded-2xl border border-red-200 bg-red-50 px-5 py-4 text-sm text-red-700">
              Unable to load this faculty.
            </div>
          ) : null}
            ...
            >
              ...
              <section className="space-y-3 border-t border-slate-100 pt-5">
                ...
                <p className="text-xs text-slate-400">
                  Select subjects for this faculty. Unselected subjects
                  currently assigned here will be unassigned.
                </p>
                ...
                {subjects.length === 0 ? (
                  ...
                  <div className="grid max-h-64 gap-2 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50/60 p-3 sm:grid-cols-2">
                    {subjects.map((subject) => {
                      const selected = selectedSubjectUuids.includes(
                        subject.uuid,
                      )
                      return (
                        <button
